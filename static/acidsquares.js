...
//
// Vanilla JS port of the React Bits "Acid Squares" background
// (Backgrounds/AcidSquares). Same OGL rendering pipeline and shaders as
// the official React component - the WebGL/OGL logic never actually
// depended on React, so this drops straight into a plain HTML page.
//
// Usage:
//   import { createAcidSquares } from "/static/acidsquares.js";
//   const destroy = createAcidSquares(document.getElementById("acid-bg"), {
//     color1: "#5227FF", ...
//   });
//   // later, if needed: destroy();

import { Renderer, Program, Mesh, Triangle, RenderTarget } from 'https://esm.sh/ogl@1.0.11';
...
/**
 * Mount an Acid Squares WebGL background into `container`.
 * Returns a destroy() function to tear it down.
 */
export function createAcidSquares(container, options = {}) {
  const cfg = {
    color1: '#5227FF',
    color2: '#A855F7',
    color3: '#FFFFFF',
  ...
  /* Spider-Man logo behind the Acid Squares */
  const spiderLogo = document.createElement('img');

  // spiderLogo.src = '/static/spider-man.jpg';
  spiderLogo.alt = '';
  spiderLogo.setAttribute('aria-hidden', 'true');

  spiderLogo.style.position = 'absolute';
  spiderLogo.style.left = '50%';
  spiderLogo.style.top = '50%';
  spiderLogo.style.transform = 'translate(-50%, -50%)';

  spiderLogo.style.width = 'min(850px, 65vw)';
  spiderLogo.style.height = 'auto';

  spiderLogo.style.opacity = '0.22';
  spiderLogo.style.zIndex = '1';

  spiderLogo.style.pointerEvents = 'none';
  spiderLogo.style.userSelect = 'none';

  /* Makes the black background of the JPG disappear */
  spiderLogo.style.mixBlendMode = 'screen';

  if (window.innerWidth <= 600) {
  spiderLogo.style.width = '75vw';
  spiderLogo.style.opacity = '0.16';
}

  container.appendChild(spiderLogo);
  ...
  const renderFrame = () => {
    const grainOn = cfg.grain ? 1.0 : 0.0;
    ...
    if (cfg.blur > 0) {
      ...
      pu.uRadius.value = cfg.blur * 14.0;
  ...
  const loop = t => {
    ...
    const activeTarget = cfg.mouseInteraction ? mouseActiveTarget : 0;
    mouseActive += 0.05 * (activeTarget - mouseActive);
    program.uniforms.uMouseActive.value = mouseActive;
    program.uniforms.uEnableMouse.value = cfg.mouseInteraction ? 1.0 : 0.0;
    program.uniforms.uMouseStrength.value = cfg.mouseStrength;
  ...
  return function destroy() {
    ...
    try {
      container.removeChild(spiderLogo);
    } catch {
      /* already removed */
    }
